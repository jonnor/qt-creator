...
DistutilsBuildSettingsWidget::DistutilsBuildSettingsWidget(DistutilsTarget *target) :
    m_target(target),
    m_pathChooser(0),
    ...
    m_buildConfiguration(0)
{
    ...
    m_pathChooser->setBaseDirectory(m_target->distutilsProject()->projectDirectory());
    ...
    connect(m_target->distutilsProject(), SIGNAL(toolChainChanged(ProjectExplorer::ToolChain*)),
...
QString DistutilsBuildSettingsWidget::displayName() const
{
    return QLatin1String("Distutils Manager");
}

void DistutilsBuildSettingsWidget::init(BuildConfiguration *bc)
{
    m_buildConfiguration = static_cast<DistutilsBuildConfiguration *>(bc);
    m_pathChooser->setPath(m_buildConfiguration->buildDirectory());
}

void DistutilsBuildSettingsWidget::buildDirectoryChanged()
{
    m_buildConfiguration->setBuildDirectory(m_pathChooser->rawPath());
}

void DistutilsBuildSettingsWidget::toolChainSelected(int index)
{
    using namespace ProjectExplorer;

    ToolChain *tc = static_cast<ToolChain *>(m_toolChainChooser->itemData(index).value<void *>());
    m_target->distutilsProject()->setToolChain(tc);
}

void DistutilsBuildSettingsWidget::toolChainChanged(ProjectExplorer::ToolChain *tc)
{
...
void DistutilsBuildSettingsWidget::updateToolChainList()
{
    m_toolChainChooser->clear();

    QList<ToolChain *> tcs = ToolChainManager::instance()->toolChains();
    if (!m_target->distutilsProject()->toolChain()) {
    ...
    foreach (ToolChain *tc, tcs) {
        ...
        if (m_target->distutilsProject()->toolChain()
                && m_target->distutilsProject()->toolChain()->id() == tc->id())
