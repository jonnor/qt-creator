 ...
 *
 * - DistutilsManager: Will manage the new distutils project and
 *   tell QtCreator for which MIME types the distutils project should
 *   be instantiated.
 *
 ...
 *
 * - DistutilsTargetFactory: Our current target is desktop.
 *
 * - DistutilsBuildConfigurationFactory: Creates build configurations that
 *   contain the steps (make, autogen, autoreconf or configure) that will
 *   be executed in the build process)
 */

class DistutilsProjectPlugin : public ExtensionSystem::IPlugin
{
    Q_OBJECT
    Q_PLUGIN_METADATA(IID "org.qt-project.Qt.QtCreatorPlugin" FILE "DistutilsProjectManager.json")

public:
    DistutilsProjectPlugin();
